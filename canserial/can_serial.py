...
class CanSerialBus(BusABC):
    ...
    def send(self, msg: Message, timeout: Optional[float] = None) -> None:
        ...
        # Pack arbitration ID
        try:
            arbitration_id = struct.pack("<h", msg.arbitration_id)
        except struct.error:
            raise ValueError(
        ...
        dlc_encoded = f"0xC{msg.dlc}"
        dlc_encoded = int(dlc_encoded,16)
        byte_msg.append(dlc_encoded)
    ...
    def _recv_internal(
        ...
        try:
            ...
            if rx_byte and ord(rx_byte) == 0xAA:
                #Byte 1: Data Frame Information
                encoded = self._ser.read(1)
                # print("ENCODED", encoded)
                dlc = encoded[0] & 15
                # print("DL",dlc)
                
                # Byte 2 & 3: Frame ID (Little Endian)
                id = self._ser.read(2)
                # The arbitration id is encoded in two bytes in little endian
                arbitration_id = int(id[::-1].hex(), 16)
                if arbitration_id >= 0x20000000:
                    raise ValueError(
                        "received arbitration id may not exceed 2^29 (0x20000000)"
                    )
                # print("ID: ", arbitration_id)
